...
- 直接内存：不是虚拟机运行时数据区的一部分，也不是java虚拟机规范中定义的内存区域：如果使用了NIO，这块区域会被频繁使用，在Java对内可以用directByteBuffer对象直接引用并操作，这块内存不受java堆大小限制，堆中只是有引用指向该区域，防止出现OOM可以设置MaxDirectMemorySize来设置。
