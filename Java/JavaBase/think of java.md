...
Comparator 接口中的比较方法是 `compare(T o1, T o2)`，其中 `o1` 和 `o2` 分别表示需要比较的两个对象，方法返回值为一个整数，表示大小比较结果。具体规则如下：

```
如果 o1 小于 o2，返回一个负整数（通常是 -1）。
如果 o1 等于 o2，返回 0。
如果 o1 大于 o2，返回一个正整数（通常是 1）。
```
...
# [24.File](./md/think of Java：File.md)
